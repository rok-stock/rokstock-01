# 로드맵 (마일스톤)

**"하루 늦은 모의주식"** — 공공데이터포털의 일별 시세(T+1 영업일 13시 갱신)를 그대로 게임 규칙으로
삼은 **모바일 모의투자 게임**을 만든다. 로그인 없이 localStorage로 동작하고, 초기 자금 1억 원으로
시작하며, 기업 리포트 등 건전한 투자 개념을 학습 요소로 담는다.

> 2026-08 방향 전환: 원래 이 프로젝트는 자동 매매 프로그램(가상 매매 → KIS 모의투자 → 실전)을
> 목표로 했다. M1까지 진행한 뒤 **게임 중심으로 전면 개편**했다. 기존 자동 매매 로드맵(구 M2~M6)은
> 문서 하단 [장기 아이디어](#장기-아이디어-구-m2m6)에 보존한다.
>
> 상태 표기: ⬜ 예정 / 🟨 진행 중 / ✅ 완료

---

## M0. 프로젝트 기반 ✅

프로젝트의 뼈대와 운영 규칙을 만든다.

- [x] Next.js 스캐폴딩 (TypeScript + Tailwind CSS v4 + App Router)
- [x] 프로젝트 규칙 문서화 (AGENTS.md)
- [x] 마일스톤 수립 (이 문서)
- [x] 화면 버전 표기 (footer의 VersionBadge)
- [x] main 브랜치 정리 및 첫 PR

**학습 목표**
- 웹개발: Next.js App Router 구조(layout/page), 서버 컴포넌트, SemVer 버전 관리
- 주식: (없음 — 기반 단계)

---

## M1. 시세 조회 대시보드 ✅

실제 KRX 시세(지연/일봉)를 조회하고 보여주는 화면을 만든다.

- [x] 종목 검색 (종목명/종목코드)
- [x] 현재가(지연 시세)와 등락률 표시
- [x] 일봉 차트 표시
- [x] 관심 종목 목록 (localStorage)

**구현 메모**
- 시세는 공급자(provider) 인터페이스 뒤에 숨겼다 → `src/lib/market/`, 자세한 내용은
  [docs/market-data.md](./market-data.md)
- 기본은 API 키가 필요 없는 **목업 공급자**, `MARKET_DATA_PROVIDER=datagokr`로 공공데이터포털
  실제 시세 전환

**학습 목표**
- 웹개발: 외부 API 연동(Route Handler), 데이터 페칭과 캐싱, 차트 라이브러리
  → [learning/web-server-client-components.md](./learning/web-server-client-components.md)
- 주식: 호가/현재가/등락률, 캔들(일봉)의 의미, 거래량
  → [learning/stock-price-basics.md](./learning/stock-price-basics.md)

---

## G0. 게임 기획 & 로드맵 개편 ✅

게임의 규칙과 작업 계획을 확정한다. (이 마일스톤부터 접두사가 G — Game)

- [x] 로드맵 전면 개편 (이 문서)
- [x] 게임 설계 문서 작성 → [docs/game-design.md](./game-design.md)
- [x] AGENTS.md 프로젝트 개요 갱신
- [x] GitHub 이슈 세트 생성 (G1~G7)

**학습 목표**
- 웹개발: (없음 — 기획 단계)
- 주식: 주문과 체결의 구분, 룩어헤드 편향(look-ahead bias) — 왜 "다음 종가 체결"이어야 게임이
  성립하는가 → [game-design.md](./game-design.md)의 체결 규칙 참조

---

## G1. 시드 데이터 파이프라인 + 공급자 개편 + ISR ✅

KOSPI 전 종목 데이터를 로컬에서 생성해 커밋하고, 운영 중 API 호출을 최소화하는 기반을 만든다.

- [x] 시드 생성 스크립트 (`scripts/build-seed.ts`) — KOSPI 종목 마스터(+법인등록번호),
      최근 5영업일 전 종목 시세 스냅샷, KOSPI 지수 1년치
- [x] 시드 산출물 커밋 (`src/data/*.json`, 튜플 포맷으로 용량 최적화 — raw 약 300KB)
- [x] 시세 공급자 개편: 전 종목 스냅샷 1회 페치가 단일 진실 공급원, 검색은 시드 로컬(API 0회),
      API 실패 시 시드 fallback
- [x] ISR 적용 (`export const revalidate` + fetch 태그) 및 온디맨드 재검증
      (`/api/revalidate` + Vercel cron, 13시 갱신 직후 반영)
- [x] Vercel 배포 설정 (vercel.json, 환경변수 문서화)

**구현 메모**
- KRX상장종목정보에는 우선주가 없어(831법인 vs 시세 943종목) 시세 스냅샷과 병합하고,
  우선주의 법인등록번호는 보통주에서 유도했다.
- 기업재무·기업기본·배당·발행정보 API는 승인 확인 후 G6에서 연동한다
  (배당·발행은 비상업 용도 사용 — [game-design.md](./game-design.md) 라이선스 절 참조).

**학습 목표**
- 웹개발: ISR(Incremental Static Regeneration), 데이터 캐시와 태그 재검증, cron
- 주식: 종목 마스터, 시가총액/상장주식수, 지수(KOSPI)의 의미

---

## G2. 게임 상태 코어 + 리셋 ✅

localStorage 기반 게임 상태와 앱 골격(하단 탭)을 만든다.

- [x] 게임 상태 스키마 (`rokstock:game` 단일 키 + schemaVersion) — 현금 1억, 포지션, 주문, 내역
- [x] `useGame` 훅 (useSyncExternalStore, 기존 useWatchlist 패턴 재사용)
- [x] 하단 탭 내비게이션 (홈 / 내역 / 설정)
- [x] 설정 화면: 게임 초기화(2단계 확인, 진행 요약 표시), 데이터 안내
- [x] 홈에 계좌 카드 (현금·시작일) + 게임 컨셉 문구

**학습 목표**
- 웹개발: localStorage 상태 설계(단일 키 원자성, 스키마 버저닝/마이그레이션), 모바일 safe-area
- 주식: 예수금(현금)과 주문가능금액

---

## G3. 주문 플로우 (접수/취소) ✅

매수/매도 주문을 접수하고 취소하는 흐름. 체결은 G4에서.

- [x] 종목 상세 하단 매수/매도 바 + 주문 바텀시트 (매수=금액 지정, 매도=수량 지정)
- [x] 주문 접수 로직 (금액/수량 잠금, `lib/game/engine.ts` 순수 함수), 미체결 주문 취소
- [x] 내역 화면의 미체결 섹션 (+ 체결 내역 UI 골격)
- [x] 첫 방문 온보딩 카드 ("왜 하루 늦은가?" 규칙 3장)
- [x] 수수료·세금·예상 체결일 안내 표시

**학습 목표**
- 웹개발: 바텀시트 UI, inputMode 등 모바일 입력 UX
- 주식: 위탁수수료와 증권거래세(농특세), 주문 유형(시장가/지정가/종가 체결)

---

## G4. 체결 엔진 ✅

접수된 주문을 다음 거래일 종가로 체결·정산한다. 게임의 심장.

- [x] 정산 순수 함수 `settleOrders` — "주문 시각 이후 처음 마감(15:30)되는 거래일의 종가" 체결.
      매수는 잠근 금액 안에서 최대 수량 매수 후 잔돈 반환(1주도 못 사면 전액 반환)
- [x] `/api/candles` Route Handler (정산·차트용 일봉)
- [x] 접속/포커스 시 자동 정산 + **체결 결과 개봉 연출** (이 게임의 리텐션 루프)
- [x] 체결 내역 완성 (수수료/세금/실현손익 명세)

**학습 목표**
- 웹개발: 순수 함수로 도메인 로직 분리, 클라이언트 주도 정산의 한계
- 주식: 평단가 계산, 평가손익 vs 실현손익

---

## G5. 홈 대시보드 완성 ✅

포트폴리오를 한눈에 보는 모바일 홈.

- [x] 총자산/수익률 카드 (현금 + 잠긴 금액 + 평가금액, 시작 자금 1억 대비 수익률)
- [x] 보유 종목 리스트 (평가손익 표시) + 미체결 배너
- [x] 종목 상세에 보유 현황 카드 (개인 상태는 클라이언트 — 페이지는 ISR 공유 캐시)
- [x] 모바일 마감 품질 (터치 타깃, safe-area, 다크모드 점검)

**학습 목표**
- 웹개발: 파생 상태 계산(valuation)과 표시 계층 분리
- 주식: 총자산 = 예수금 + 평가금액, 수익률 계산 기준

---

## G6. 기업 리포트 ✅

"싸 보여서"가 아니라 "알고" 사는 습관을 위한 기업 정보 화면.

- [x] 시드 파이프라인 확장 (`--only=financials`) — 금융위 기업재무정보/기업기본정보,
      법인등록번호(crno)로 조인 (831법인, 연 단위 데이터라 런타임 API 호출 0회)
- [x] 종목 상세 리포트 섹션: 기업 개요(대표·설립·종업원·평균급여·감사의견), 3개년 요약 재무
- [x] PER/PBR 계산 표시 (시가총액 ÷ 순이익/자본총계) + 용어 카드
- [x] 배당수익률·주당배당금(최근 1년 현금배당 합산)·액면가 — 주식배당정보 API.
      발행정보 API는 불필요해짐(액면가가 배당 응답에 포함)

**학습 목표**
- 웹개발: 정적 데이터의 빌드 타임 번들 vs 런타임 페치 트레이드오프
- 주식: 재무제표 읽기 기초, PER/PBR, 가치평가의 한계

---

## G7. 벤치마크 + 재미 요소 ✅

"내가 잘한 건가?"에 답을 주는 비교 기준과 게임적 재미.

- [x] KOSPI 벤치마크 비교 — "게임 시작일에 전액 KOSPI 지수에 넣었다면 지금 얼마?"
      (지수시세정보 API + 시드 폴백, `/api/index`)
- [x] 업적 시스템 6종 (첫 매수/매도/수익, 분산투자 5종목, 한 달 생존, 수수료로 치킨 —
      스키마 v1→v2 마이그레이션 실전 사례)
- [x] ConceptTip 용어 카드 (평단가·실현손익·수수료·벤치마크 등 탭하면 짧은 설명)

**학습 목표**
- 웹개발: localStorage 스키마 마이그레이션 실전
- 주식: 벤치마크와 인덱스 투자, 분산투자의 의미

---

## G8. 데스크톱 대응 ✅

모바일 우선으로 만든 게임을 PC 화면에도 최적화한다. 전환점은 lg(1024px) 하나로 통일.

- [x] 공용 컨테이너 `container-page` (모바일 768px → 데스크톱 1024px, Tailwind v4 @utility)
- [x] 데스크톱 내비: 헤더에 홈/내역/설정 (TabBar 와 TABS 공유) + sticky 헤더, 탭바는 lg 에서 숨김
- [x] 홈·종목 상세 2컬럼 (상세는 우측 sticky 사이드바에 가격·보유·주문 카드)
- [x] 바텀시트 3종(주문/개봉/용어) → 데스크톱 중앙 모달
- [x] 차트 높이 확대 (450px → lg 560px)
- [x] 키보드: 전역 :focus-visible 링, 모달 Esc 닫기(useEscapeClose), 검색 Esc / hover 누락 보강
- [x] 검색 드롭다운 방향키/Enter 콤보박스 (ARIA combobox + aria-activedescendant)

**학습 목표**
- 웹개발: 모바일 퍼스트 반응형, fixed 의 DOM 위치 독립성, grid order, :focus-visible
  → [learning/web-responsive-desktop.md](./learning/web-responsive-desktop.md)
- 주식: (없음 — UI 단계)

---

## G9. 공유·PWA ✅

친구를 데려오는 문과, 홈 화면에 자리 잡는 아이콘.

- [x] 설정에 공유하기 — Web Share API(OS 기본 공유 시트), 미지원 브라우저는 클립보드 복사 폴백
- [x] OG 프리뷰 카드 — `opengraph-image.tsx`(1200×630, 한글 폰트 서브셋 + 캔들 실루엣) +
      openGraph/twitter 메타
- [x] PWA 매니페스트 — 설치 이름 "하루 늦은 모의주식" / 홈 화면 "하루주식"(짧게),
      🐢 아이콘(512 maskable 겸용 + 애플 180)

**학습 목표**
- 웹개발: Web Share API 와 폴백 설계, Open Graph 프로토콜, PWA 매니페스트와 maskable 아이콘
  → [learning/web-share-og-pwa.md](./learning/web-share-og-pwa.md)
- 주식: (없음 — UI 단계)

---

## G10. 즉시 체결로 전환 ✅

G4에서 만든 "다음 거래일 종가 체결"(미체결 주문 → 익일 정산 → 체결 개봉 연출)을 되돌리고,
**조회 시점 최신 종가로 그 자리에서 즉시 체결**되도록 게임 규칙을 바꿨다.

- [x] `lib/game/engine.ts` — `placeBuy`/`placeSell`/`cancelOrder`/`settleOrders`/
      `expectedExecDate` 제거, `buyNow`/`sellNow` 로 교체(그 자리에서 `Trade` 반환)
- [x] 상태 스키마 v2 → v3 — `pendingOrders`/`lockedCash`/`Position.lockedQuantity` 제거,
      마이그레이션에서 남은 미체결 주문은 취소 처리(잠긴 현금 반환)
- [x] `GameSettlement`(정산 트리거 + 개봉 시트) 제거 → `AchievementChecker`(시간 경과형
      업적만 점검) + `TradePanel`이 매수/매도 직후 체결 결과를 토스트로 즉시 표시
- [x] `/api/candles` Route Handler 제거(정산 전용이라 더 쓰이지 않음)
- [x] 홈/내역/온보딩/설정 문구를 "다음 거래일 체결" → "즉시 체결"에 맞게 갱신

**학습 목표**
- 웹개발: 스키마 마이그레이션으로 필드 제거(추가만이 아니라 축소도 다뤄야 함)
- 주식: 룩어헤드 편향을 원천 차단하는 설계와, 그걸 의식적으로 완화하는 트레이드오프의 차이
  → [learning/stock-order-execution.md](./learning/stock-order-execution.md)

---

## G11. 시장 학습 뷰 4종 ✅

보유 데이터(전종목 시세·재무·배당·지수)로 "주식 공부에 도움이 되는" 탐색 화면을 만들었다.
`getAllQuotes()`(구현만 있고 안 쓰이던 전종목 스냅샷)가 처음으로 화면에 연결됐다.

- [x] `/market` **시장 랭킹** — 상승률/하락률/거래량 TOP 30. 서버가 세 리스트를 미리 계산해
      ISR 로 굽고, 탭 전환은 클라이언트에서 배열만 바꾼다 (searchParams 를 쓰면 동적
      렌더링으로 떨어져 ISR 이 깨진다는 함정을 문서로 확인 후 회피)
- [x] `/market/screener` **밸류에이션 스크리너** — 보통주 ~780종목 × PER/PBR/배당수익률/시총
      정렬. 계산 로직은 `lib/market/metrics.ts` 로 추출해 종목 상세(CompanyReport)와 단일
      소스화. mock 공급자에 결정적 시가총액 보강(시드의 발행주식수 × 목업 가격)
- [x] `/market/index` **KOSPI 지수 페이지** — 1년치 영역 차트(+1년 수익률/최고/최저),
      홈 벤치마크 카드에서 링크
- [x] `/watchlist` **관심종목 수익률 비교** — 구간 첫 종가 대비 % 정규화 라인 차트,
      기간(1/3/6/12개월)·종목 숨김 토글, CVD 검증 8색 고정 팔레트.
      `/api/candles?codes=` 배치 일봉 라우트 신설(G10 에서 삭제된 정산용과 경로만 같음)
- [x] 하단 탭 "시장" 추가(4탭), `/market` 서브내비 pill, GLOSSARY 에 시가총액·코스피지수

**학습 목표**
- 웹개발: ISR 와 Request-time API(searchParams)의 상충, 서버 1회 계산 + 클라이언트
  재정렬 패턴, canvas 차트의 접근성 대체(표 병행)
- 주식: 랭킹 추격 매수의 함정, 저PER/저PBR 이 "싸다"가 아닌 이유, 수익률 정규화(지수화),
  시가총액 vs 주가 → [learning/stock-screener-basics.md](./learning/stock-screener-basics.md)

---

## G12. 사용자 행동 분석 (GA4 + GTM) ✅

Google 애널리틱스 4 와 태그 관리자를 붙여 사용자 행동을 측정한다.

- [x] GA4 속성 생성 (계정 `rokstock` > 속성 `rokstock-01`, 측정 ID `G-72NP373Q8J`,
      향상된 측정 ON — SPA 라우팅 page_view 자동)
- [x] GTM 컨테이너 생성·게시 (`GTM-5D6J39X9`) — Google 태그(페이지뷰) +
      GA4 이벤트 태그(맞춤 이벤트 trade/achievement/share, dataLayer 변수 7개 매핑)
- [x] 코드: `@next/third-parties` `<GoogleTagManager>` (env `NEXT_PUBLIC_GTM_ID` 조건부,
      Vercel Production 전용), `lib/analytics.ts` 이벤트 헬퍼(미사용 파라미터 null 리셋),
      TradePanel/AchievementModal/ShareButton 계측
- [x] 종단 검증: dataLayer push(gtm.uniqueEventId 부여) → GA4 실시간 수신 확인

**학습 목표**
- 웹개발: GA4 vs GTM 역할 분리, dataLayer 계약, "GTM은 커스텀 이벤트를 자동으로 GA4에
  안 보낸다" 함정 → [learning/web-analytics-ga4-gtm.md](./learning/web-analytics-ga4-gtm.md)
- 주식: (없음 — 인프라 단계)

---

## 장기 아이디어 (구 M2~M6)

게임이 완성된 뒤에 검토할 원래 로드맵의 아이디어들. 게임의 데이터 파이프라인과 도메인 로직
(체결/평가) 위에 얹을 수 있다.

- **자동 매매 엔진 (가상)** — 전략 인터페이스(이동평균 교차, RSI), 전략 실행기, 스케줄링, 매매 로그
- **백테스팅** — 과거 일봉 수집/저장, 백테스트 실행기, 성과 지표(누적 수익률/MDD/승률)
- **KIS 모의투자 연동** — 한국투자증권 Open API 모의투자 계좌, 토큰 인증, 잔고/주문/체결 조회
- **실전 매매** — 실계좌 전환, 리스크 관리(손절 라인/한도), 모니터링·감사 로그
