...
게임이 업데이트되면 저장된 상태의 **구조가 달라질 수 있다** (예: G7 에서 업적 필드 추가).
그래서 상태에 `schemaVersion: 1` 을 박아두고, 읽을 때 `migrate()` 를 거친다:

- 버전이 맞고 형태가 온전하면 → 그대로 사용
- 모르는 버전/깨진 JSON 이면 → 새 게임으로 초기화 (v1의 정책)
- 다음 버전부터는 여기서 `v1 → v2` 변환 코드를 추가한다
